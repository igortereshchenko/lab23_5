...
if ENV == 'dev':
    app.debug = True
    # app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://Vano:123123@93.72.185.72:5431/ivan'
...
def create_cities():
    ...
    db.session.add_all([BerlinEx, KyivEx, NYEx])
    db.session.commit()
...
@app.route('/shop')
def display_cities():
    res = db.session.query(OrmCity).all()

    return render_template('city_table.html', cities=res)
...
def create_city():
    ...
    if request.method == 'POST':
        new_city = OrmCity(
            ...
            city_coord=form.city_coord.data,
            city_year=form.city_year.data
        )
...
def plot():
    query1 = (
        db.session.query(
            OrmCity.city_name,
            OrmCity.city_population.label('popul'))
    ).all()

    city_name, city_population = zip(*query1)
    bar = go.Bar(
        x=city_name,
        y=city_population
    )
